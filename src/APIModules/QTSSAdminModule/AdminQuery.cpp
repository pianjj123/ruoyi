...
这段代码是一个Objective-C++文件，用于实现一个管理查询（Admin Query）的功能。它主要包含一个名为QueryURI的类，该类用于解析和管理HTTP请求中的URI（统一资源标识符）。

实现原理
类定义：QueryURI类定义了多个成员变量，用于存储解析后的URI字段和查询参数。
URI解析：URLParse方法负责解析HTTP请求中的URI，并将其分解为不同的字段，如模块（modules）、根（admin）、URL、查询参数等。
查询参数解析：ParseQueryString方法负责解析URI中的查询参数，并将其存储在相应的成员变量中。
命令处理：EvalQuery方法根据解析出的命令（如GET、SET、ADD、DEL）执行相应的操作。
辅助方法：类中还包含了一些辅助方法，如NextSegment用于获取URI的下一个段，SetSnapShot、SetCommand、SetAccessFlags、SetParamBits等方法用于设置查询的快照、命令、访问标志和参数位。
用途
该代码主要用于处理HTTP请求中的管理查询，可以用于配置管理、状态查询等场景。通过解析HTTP请求中的URI和查询参数，可以实现对系统配置、状态等的动态查询和修改。

注意事项
线程安全：如果该代码在多线程环境中使用，需要确保线程安全，可能需要使用互斥锁（Mutex）来保护共享资源。
错误处理：代码中包含了一些错误处理逻辑，例如检查无效的迭代器、递归参数等，以防止不合法的查询操作。
内存管理：代码中使用了动态内存分配（如NEW关键字），需要确保在适当的时候释放内存，以避免内存泄漏。
兼容性：代码中使用了#ifndef __Win32__宏来包含不同的头文件，可能需要根据目标平台进行调整。
总的来说，这段代码是一个功能较为复杂的HTTP请求解析器，适用于需要动态管理配置和状态的应用场景。
